...
type FeedbackItem = TextEdit | Comment;
...
function getSelector(el: Element): string {
  ...
  while (current && current !== document.body) {
    ...
    if (current.className && typeof current.className === "string") {
      const classes = current.className.split(" ").filter(c => c && !c.startsWith("__")).slice(0, 2).join(".");
...
function isTextElement(el: Element): boolean {
  ...
  // Check if it has mostly text content (not complex nested elements)
...
export default function FeedbackWidget() {
  ...
  useEffect(() => {
    setIsVisible(window.location.hostname === "localhost");
    loadFeedbackCount();
  }, []);
  ...
  const handlePageClick = (e: MouseEvent) => {
    ...
    if (mode === "editing" && isTextElement(target)) {
      ...
      // Show comment input for any element
      setActiveElement(target);
  ...
  return (
    <div id="feedback-widget" className="fixed bottom-4 right-4 z-[9999] font-sans">
      {/* Toast */}
      {showToast && (
      ...
      {mode === "commenting" && activeElement && (
        <div className="w-72 rounded-lg bg-gray-900 p-4 shadow-xl">
          <p className="mb-2 text-xs text-gray-400 truncate">
